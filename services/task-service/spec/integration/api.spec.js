...
describe('API Integration Tests', () => {
    ...
    describe('POST /items', () => {
        test('creates a new item', async () => {
            const res = await request(app).post('/items').send({ name: 'Buy groceries' });
            expect(res.status).toBe(200);
        ...
        test('created item is persisted', async () => {
            await request(app).post('/items').send({ name: 'Persisted item' });
            const res = await request(app).get('/items');
        ...
        test('each item gets a unique id', async () => {
            const res1 = await request(app).post('/items').send({ name: 'Item A' });
            const res2 = await request(app).post('/items').send({ name: 'Item B' });
    ...
    describe('PUT /items/:id', () => {
        test('updates an existing item', async () => {
            const created = await request(app).post('/items').send({ name: 'Original' });
        ...
        test('update is persisted', async () => {
            const created = await request(app).post('/items').send({ name: 'Before' });
            await request(app).put(`/items/${created.body.id}`).send({ name: 'After', completed: true });
            const res = await request(app).get('/items');
            expect(res.body.length).toBe(1);
        ...
        test('toggling completed back to false works', async () => {
            const created = await request(app).post('/items').send({ name: 'Toggle me' });
            await request(app).put(`/items/${created.body.id}`).send({ name: 'Toggle me', completed: true });
            const res = await request(app).put(`/items/${created.body.id}`).send({ name: 'Toggle me', completed: false });
            expect(res.body.completed).toBe(false);
        });
    });
    ...
    describe('DELETE /items/:id', () => {
        ...
        test('deleted item is no longer returned', async () => {
            const created = await request(app).post('/items').send({ name: 'Will vanish' });
        ...
        test('deleting one item does not affect others', async () => {
            await request(app).post('/items').send({ name: 'Keep me' });
            const item2 = await request(app).post('/items').send({ name: 'Delete me' });
    ...
    describe('Full CRUD cycle', () => {
        test('create → read → update → read → delete → read', async () => {
            const created = await request(app).post('/items').send({ name: 'Full cycle' });
            ...
            await request(app).put(`/items/${created.body.id}`).send({ name: 'Full cycle - done', completed: true });
